...
$passwordReconfirm.on('input',function(){
    ...
    function passwordCheck() {
        return ($password.val() !== '') && ($passwordReconfirm.val() === $password.val() && regPassword.test($password.val()) ) ;
    }
    ...
    $.ajax({
        ...
        success: function (result) {
            if(result){
                ...
                $authentication.html("<input type='button' id='use-email' value='이 이메일 사용할래요'/>");
            }
        }
    })
});
...
$authentication.on('click','#use-email',function(){
    ...
    $check.text('');
    $authentication.append("<input type='button' id='auth-code-certified' value='인증' style='height:40px;margin-left:20px;'/>");
