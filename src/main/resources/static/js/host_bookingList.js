...
async function fetchReservations(status) {
    try {
        ...
        const filteredReservations = reservations.filter(reservation => {
            if (status === '') return true; // 모든 예약 가져오기
            if (status === 'using_before' && reservation.status === 1) return true; // 계약 완료 상태
            if (status === 'using_after' && reservation.status === 3) return true; // 계약 만료 상태
