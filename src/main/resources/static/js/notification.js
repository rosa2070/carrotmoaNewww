...
const isNotificationInitialized = localStorage.getItem(
    "isNotificationInitialized") === "true";
let storedNotifications = JSON.parse(localStorage.getItem('notifications'))
    || [];
let modifiedNotifications = JSON.parse(
    localStorage.getItem("modifiedNotifications")) || [];
...
// login이 된 상태면 -> SSE 연결하기.
  ...
  const allNotificationsDeleted = localStorage.getItem(
      "allNotificationsDeleted") === "true";
...
function notificationStorageList(notifications) {
  ...
  notifications.forEach(notification => {
    ...
    const notificationClass = notification.read ? 'notification-url read'
        : 'notification-url';
    ...
    notificationEntry.innerHTML = `
            <a class="${notificationClass}" href="${notification.url}">
                ...
                <div class="notification-details">
                    ...
                    <div class="notification-createdAt">${formatElapsedTime(
        notification.createdAt)}</div>
                </div>
            </a>
    ...
    // 실시간 경과 시간 업데이트
    setInterval(() => {
      const timeElement = notificationEntry.querySelector(
...
function addModifiedNotification(id, isRead = null, isDeleted = null) {
  const existingNotification = modifiedNotifications.find(
      notif => notif.id === id);
  ...
  if (existingNotification) {
    ...
    if (isDeleted !== null) {
      existingNotification.isDeleted = isDeleted;
    }
  } else {
    modifiedNotifications.push({id, isRead, isDeleted});
  }

  localStorage.setItem("modifiedNotifications",
      JSON.stringify(modifiedNotifications));
}
...
document.addEventListener("DOMContentLoaded", function () {
  notificationList.addEventListener("click", function (event) {
    ...
    if (deleteIcon) {
      ...
      const deleteButton = notificationEntry.querySelector(
          ".notification-delete-button");
    ...
    if (deleteButton) {
      ...
      const notificationRead = storedNotification ? storedNotification.read
          : false;
    ...
    if (notificationLink) {
      ...
      const notificationId = Number(notificationEntry.getAttribute("data-id"));
      const notificationIsDeleted = notificationEntry.getAttribute(
          "data-deleted");
      ...
      if (storedNotification) {
        ...
        localStorage.setItem('notifications',
            JSON.stringify(storedNotifications)); // 업데이트된 데이터를 localStorage에 저장
      }
      // TODO: 알림 읽는 것 추가.
