...
async function fetchPostsBySubCategory(subcategoryId) {
    ...
    try {
        const response = await fetch(`/api/community/posts/subCategories/${subcategoryId}?page=${subcategoryCurrentPage}&size=${communityPostsSize}`); // 초기 페이지 요청
        ...
        if (subcategoryIsLastPage) {
            ...
            if (!document.getElementById('subcategory-no-more-posts')) {
                ...
                const categoryName = data.content.length > 0 ? data.content[0].categoryName : '이 카테고리';
...
async function fetchPosts() {
    ...
    try {
        // 서버로부터 데이터를 가져오는 부분
        const response = await fetch(`/api/community/posts?page=${currentPage}&size=${communityPostsSize}`);
        // JSON 형식으로 변환
        const data = await response.json();
        console.log(data);
        // 게시글 리스트와 마지막 페이지 여부
