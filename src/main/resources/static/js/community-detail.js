      ...
      <div class="detail-comment-header">
        ...
        <div class="comment-user-info">
          <div class="detail-comment-nickname">
            ...
            ${isPostWriter
      ? `<img src="/images/community/post-writer.svg" alt="게시글 작성자 댓글" id="postWriter" class="post-writer">`
      : ''}
          </div>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  commentsList.addEventListener("click", function (event) {
    if (event.target && event.target.closest(".reply-button")) {
      const commentId = event.target.closest(".reply-button").getAttribute(
          "data-comment-id");
      const replySection = event.target.closest(
          ".detail-comment-wrap").querySelector(".reply-input-section");

      // display가 none이면 block으로, block이면 none으로 전환
      replySection.style.display = replySection.style.display === "none"
      || replySection.style.display === "" ? "block" : "none";
  ...
  commentsList.addEventListener("submit", function (e) {
    ...
    if (e.target && e.target.id === "replyCommentForm") {
      ...
      const commentId = replyForm.closest(".detail-comment-wrap").querySelector(
          ".reply-button").getAttribute("data-comment-id");
      // 답글 등록 API 호출
      fetch(
          `/api/community/posts/${communityPostId}/comments/${commentId}/replies`,
          {
            method: "POST",
            headers: {
      ...
      .then(data => {
        ...
        // 댓글 리스트를 다시 불러옵니다.
        return fetch(`/api/community/posts/${communityPostId}/comments`,
            {method: "GET"});
      })
      .then(response => {
        if (!response.ok) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  commentsList.addEventListener("click", function (event) {
    ...
    if (target && target.classList.contains("deleteComment")) {
      ...
      const commentId = target.closest(".detail-comment-wrap").getAttribute(
          "data-comment-id");
      fetch(`/api/community/posts/${communityPostId}/comments/${commentId}`,
          {method: "DELETE"})
      .then(response => {
        if (!response.ok) {
