...
function validateForm() {
    // 각 필드 유효성 검사
    if (!validateField('title', '숙소 이름은 필수 입력입니다.')) return false;
    if (!validateField('roadAddress', '도로명 주소는 필수 입력입니다.')) return false;
    if (!validateField('lotAddress', '지번 주소는 필수 입력입니다.')) return false;
    if (!validateField('detailAddress', '상세 주소는 필수 입력입니다.')) return false;
    if (!validateField('floor', '층 수는 필수 입력입니다.', true)) return false; // 숫자 검증 추가
    if (!validateField('totalFloor', '총 층 수는 필수 입력입니다.', true)) return false; // 숫자 검증 추가
    if (!validateField('totalArea', '총 면적은 필수 입력입니다.', true)) return false; // 숫자 검증 추가
    if (!validateField('price', '가격은 필수 입력입니다.', true)) return false; // 숫자 검증 추가
    if (!validateField('content', '상세 설명은 필수 입력입니다.')) return false;
    if (!validateField('transportationInfo', '교통 정보는 필수 입력입니다.')) return false;
...
function handleFormSubmission(event) {
    if (!validateForm()) return; // 유효성 검사 실패 시 함수 종료
