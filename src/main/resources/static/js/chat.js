...
window.onload = function () {
  ...
  if (document.getElementById('chat-modal-open')) {
    document.getElementById('chat-modal-open').addEventListener('click',
        function () {
          ...
          .then(data => {
            ...
            function connect(chatRoomId) {
              ...
              stompClient.connect({}, function (frame) {
                ...
                stompClient.subscribe('/sub/chat/' + chatRoomId,
            ...
            function sendMessage() {
              ...
              inputMessage.value = document.getElementById(
                  'input-message').value;
              ...
              stompClient.send('/pub/chat/' + chatRoomId, {},
                  JSON.stringify(data));
              inputMessage.value = "";
            }
            ...
            async function getChatMessage(roomId) {
              ...
              .then(async message => {
                ...
                document.getElementById(
                    'chat-room-id').value = message[0].chatRoomId;
                //메시지에 넣어줄 닉네임 추가
                userNicknameMap = await getNickname(userId, chatRoomId);
                message.forEach(m => {
            ...
            document.getElementById('chat-find-user').addEventListener('click',
                function () {
                  let searchNickname = document.getElementById(
                      'chat-find-user-search').value;
                  chatFindUser(searchNickname);
                });

            function chatFindUser(searchKeyword) {
              //userId  , nickname
              fetch(`/api/user/find-user/nickname/${encodeURIComponent(
                  searchKeyword)}`, {
                method: "GET",
                headers: {
                  'Content-Type': 'application/json'
              ...
              .then(result => {
                ...
                document.getElementById(
                    'chat-find-result-id').value = result.userId;
                document.getElementById(
                    'chat-find-result-nickname').textContent = result.nickname;
              });
            }

            document.getElementById('chat-room-create').addEventListener(
                'click', function () {
                  let joinTargetUserId = document.getElementById(
                      'chat-find-result-id').value;
            ...
            function createChatRoom(myUserId, joinTargetUserId) {
              fetch(`/api/chat/create-room?myUserId=${encodeURIComponent(
                  myUserId)}&joinTargetUserId=${encodeURIComponent(
                  joinTargetUserId)}`, {
                method: 'GET',
                headers: {
                  'Content-Type': 'application/json'
  ...
  async function getNickname(userId, chatRoomId) {
    try {
      return await simpleFetch(
          `/api/chat/find-chat-nickname/${userId}/${chatRoomId}`, "GET",
          {'Content-Type': 'application/json'});
    } catch (error) {
