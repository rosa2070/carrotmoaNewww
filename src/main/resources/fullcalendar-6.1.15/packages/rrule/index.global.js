...
FullCalendar.RRule = (function (exports, core, rruleLib, internal) {
  ...
  const recurringType = {
    ...
    expand(eventRRuleData, framingRange, dateEnv) {
      ...
      if (eventRRuleData.isTimeZoneSpecified) {
        ...
        dates = eventRRuleData.rruleSet.between(framingRange.start,
            framingRange.end, true);
      }
      return dates;
    },
  };
  ...
  function parseEventRRule(eventProps, dateEnv) {
    ...
    for (let exdateInput of exdateInputs) {
      ...
      rruleSet.exdate(new Date(
          res.marker.valueOf() - (res.timeZoneOffset || 0) * 60 * 1000));
    }
  ...
  function parseRRuleObject(rruleInput, dateEnv) {
    ...
    function processDateInput(dateInput) {
      if (typeof dateInput === 'string') {
        ...
        if (markerData) {
          ...
          isTimeZoneSpecified = isTimeZoneSpecified || markerData.timeZoneOffset
              !== null;
          return new Date(
              markerData.marker.valueOf() - (markerData.timeZoneOffset || 0)
              * 60 * 1000); // NOT DRY
        }
        return null;
      }
    ...
    return {
      rrule: new rruleLib__namespace.RRule(rruleOptions),
      isTimeSpecified,
      isTimeZoneSpecified
    };
  }
  ...
  function analyzeRRuleString(str) {
    ...
    function processMatch(whole, introPart, datePart) {
      ...
      isTimeZoneSpecified = isTimeZoneSpecified || result.timeZoneOffset
          !== null;
    }
