...
FullCalendar.GoogleCalendar = (function (exports, core, internal) {
  ...
  const eventSourceDef = {
    ...
    fetch(arg, successCallback, errorCallback) {
      ...
      if (!apiKey) {
        ...
        let extraParamsObj = typeof extraParams === 'function' ? extraParams()
            : extraParams;
        let requestParams = buildRequestParams(arg.range, apiKey,
            extraParamsObj, dateEnv);
        return internal.requestJson('GET', url, requestParams).then(
            ([body, response]) => {
              if (body.error) {
                errorCallback(new core.JsonRequestError(
                    'Google Calendar API: ' + body.error.message, response));
              } else {
                successCallback({
                  rawEvents: gcalItemsToRawEventDefs(body.items,
  ...
  function parseGoogleCalendarId(url) {
    ...
    if ((match = /^https:\/\/www.googleapis.com\/calendar\/v3\/calendars\/([^/]*)/.exec(
            url)) ||
  ...
  function buildUrl(meta) {
    ...
    return apiBase + '/' + encodeURIComponent(meta.googleCalendarId)
        + '/events';
  }
