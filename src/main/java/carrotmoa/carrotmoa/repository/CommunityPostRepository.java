...
public interface CommunityPostRepository extends JpaRepository<CommunityPost, Long> {
    ...
    @Query("SELECT new carrotmoa.carrotmoa.model.response.CommunityPostDetailResponse(cp.id, cp.postId, cp.communityCategoryId, p.userId, p.title, p.content, p.isDeleted, cat.name, up.nickname, up.picUrl, ua.region2DepthName, ua.region3DepthName, p.createdAt, p.updatedAt) " +
